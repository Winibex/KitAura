// lib/features/proposal/dashboard/view/prop_card_widget.dart
...
class PropCardWidget extends StatelessWidget {
  final PropSummaryModel prop;
  final VoidCallback onTap;
  final VoidCallback onDelete;
  final ValueChanged<String> onRename;

  const PropCardWidget({
    super.key,
    required this.prop,
    required this.onTap,
  ...
  Widget _buildThumbnail() {
    return Container(
      ...
      child: (prop.items != null && prop.items!.isNotEmpty)
          ? TemplateThumbnail.fromJson(
        json: {
          'canvasBackground': prop.canvasBackground ?? '#FFFFFF',
          'items': prop.items,
        },
        width: double.infinity,
        height: 300,
    ...
    return const Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 8),
          Text('Untitled',
              style: TextStyle(color: AppColors.almondSilk, fontSize: 12)),
        ],
      ),
    );
  }
  ...
  Widget _buildInfo(BuildContext context) {
    // Build subtitle from clientName / projectScope
    final subtitleParts = <String>[];
    if (prop.projectScope != null && prop.projectScope!.isNotEmpty) {
      subtitleParts.add(prop.projectScope!);
    }
    if (prop.clientName != null && prop.clientName!.isNotEmpty) {
      subtitleParts.add(prop.clientName!);
    }
    ...
    return SizedBox(
      ...
      child: PopupMenuButton<String>(
        icon: const Icon(LucideIcons.moreHorizontal,
            color: AppColors.petalFrost, size: 16),
        ...
        itemBuilder: (_) => [
          const PopupMenuItem(
            ...
            child: Row(
              children: [
                Icon(LucideIcons.pencil, size: 14, color: AppColors.slateGrey),
                SizedBox(width: 8),
                Text('Rename',
                    style: TextStyle(
  ...
  void _showRenameDialog(BuildContext context) {
    final controller = TextEditingController(text: prop.title);
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        shape:
        RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: const Text('Rename Proposal',
            style: TextStyle(
                fontFamily: AppFonts.poppins,
