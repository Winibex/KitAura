// lib/features/cover_letter/editor/view/cl_editor_screen.dart
//
// MVC VIEW — UI only. All business logic is in ClEditorController.
// Mirrors cv_editor_screen.dart with CL-specific navigation, export, and AI wiring.
...
class ClEditorScreen extends ConsumerStatefulWidget {
  final String docId;
  const ClEditorScreen({super.key, required this.docId});

  @override
  ConsumerState<ClEditorScreen> createState() => _ClEditorScreenState();
}
...
class _ClEditorScreenState extends ConsumerState<ClEditorScreen> {
  ...
  void _onEditorStateChange() {
    ...
    if (s.firestoreDocId != null && s.firestoreDocId != _lastKnownDocId) {
      ...
      if (widget.docId != s.firestoreDocId) {
        GoRouter.of(context).replace('/cover-letters/edit/${s.firestoreDocId}');
      }
    }
  ...
  Widget _buildAppBar(ClEditorState s) {
    return EditorAppBar(
      title: s.title,
